...
```
**Slug / URL:** by default the slug comes from the entry's existing link
(`blog/<slug>/`, or a legacy `blog-<slug>.html`), else a short SEO slug from the
title. For a long title, pass `--slug "zisk-zkvm-trace-generation"` to set a
short, keyword-front-loaded URL (better for SEO — search results truncate long
URLs mid-word).
...
(→ `blog/<slug>/`), and `thumbnail` (localized if it was a remote URL).
...
- Serve with the `static` preview config and open `/blog/<slug>/` and
...
- Posts live at **`blog/<slug>/index.html`** (served as `/blog/<slug>/`) with
  root-absolute asset paths. Don't relocate them.
